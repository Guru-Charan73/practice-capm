...
using {cuid,Country,Currency,managed} from '@sap/cds/common';

using {test.commons} from './commons';

context master {
    entity businesspartner {
        key NODE_KEY      : commons.Guid
            @title: '{i18n>PARTNER_KEY}';
            BP_ROLE       : String(2);
    ...
    entity product {
        key NODE_KEY       : commons.Guid
            @title: '{i18n>PRODUCT_KEY}';
            PRODUCT_ID     : String(28);
            TYPE_CODE      : String(2);
            CATEGORY       : String(32);
            DESCRIPTION    : localized String(255)
            @title: '{i18n>PRODUCT_NAME}';
    ...
    entity employees : cuid {
        ...
        sex           : commons.Gender;
        language      : String(1);
        phoneNumber   : commons.PhoneNum;
        email         : commons.Email;
        loginName     : String(12);
        Currency      : Currency;
        salaryAmount  : commons.AmountD;
...
context transaction {
    entity purchaseorder : commons.amount, cuid {
        PO_ID            : String(40)
                           @title: '{i18n>PO_ID}';
        PARTNER_GUID     : Association to one schema.master.businesspartner
                           @title: '{i18n>PARTNER_KEY}';
    ...
    entity poitems : commons.amount, cuid {
        PARENT_KEY   : Association to one purchaseorder
                       @title: '{i18n>PO_NODE_KEY}';
        PO_ITEM_POS  : Integer
                       @title: '{i18n>ITEM_POS}';
        PRODUCT_GUID : Association to one schema.master.product
